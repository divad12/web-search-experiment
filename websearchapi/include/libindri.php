...
<body>
Extension libindri_php not loaded.  Check php.ini configuration. ?>
</body>
</html>
<?php
  return;
...
include( "include/libindri_php.php") ;
...
function indri_cleanquery( $query ) {
  ...
  return shell_exec("/usr/bin/perl -w query.pl '$query'");
}

function indri_cleanquery_bow( $query ) {
  // only does bag of words if input query has no plus or quotes... bogus
  $query = str_replace( "'", "", $query ) ;
  return shell_exec("/usr/bin/perl -w query-bow.pl '$query'");
}
...
function indri_nextlink( $request, $startdoc, $phpPage = "query.php" ) {
   $server = "";
   $index = "";

   if (isset($request['query']) && $request['query'] != "None") {
      $query = "&query=" . urlencode($request['query']) ;
   }

   return "$phpPage?startdoc=" . $startdoc . $query ;
}
...
function indri_printlinks( $request, $startdoc, $resultCount,  $pagedocs, $phpPage = "query.php" ) {
  $nextlink = indri_nextlink( $request, $startdoc + $pagedocs, $phpPage );
  $prevlink = indri_nextlink( $request, $startdoc - $pagedocs, $phpPage );
